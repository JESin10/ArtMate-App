...
사용자가 문화예술 정보를 단순히 조회하는 것을 넘어 콘텐츠를 생성하고 상호작용할 수 있도록 설계한 모바일 플랫폼입니다.

한국문화정보원에서 제공하고 있는 현재 전시 작품에 대한 설명을 일괄적으로 확인하고, 전시장소를 현재 위치를 기반으로 지도 및 리스트형식으로 알 수 있는 정보제공형 플랫폼입니다.

사용자는 감상했던 전시에 대한 리뷰 및 평점을 남겨 평점별 전시 검색과 같은 특정 검색이 가능합니다.

사용자간의 팔로우, 좋아요, 댓글과 같은 상호작용이 가능하며, 팔로잉과 좋아요의 경우 상호작용이 실시간으로 가능하도록 알림을 전달하고 있습니다.

단순히 API로 문화예술 정보를 제공받기만 하는 일방형 서비스가 아닌, 공공 API를 활용해 사용자간의 콘텐츠 생성 및 상호작용이 가능한 양방향서비스로 개발하고자 하였습니다.


## 📌 서비스 아키텍쳐

초기 구조는 기능동작에 있었으나 리팩토링을 하며 유지보수성은 물론, 서비스 확장에 따른 전역관리 및 구조 분리에 초점을 맞춰 아키텍쳐를 구성하였습니다.

- Service Layer
: 데이터 접근 로직 분리
: Firebase 및 API 호출을 service로 분리해 제어하고, 재사용성과 테스트 가능성 확보

- Custom Hooks
: 비지니스 로직 캡슐화
: 데이터 흐름에따라 자주 사용되는 함수를 hook으로 분리해 컴포넌트 단순화

- Zustand Store
: 전역상태관리 및 props driling 방지
: 단순히 state에서 그치지않고, 전역 상태를 사용해 보다 간결하게 관리하고 props drilling 방지
...
또한, add, set, update, delete와 같은 기본적인 기능만 썼던 기존의 웹앱과 달리 모바일앱에서는 where, orderBy와 같은 frontend단 정렬을 사용하였습니다.

초기에는 fetch를 기반으로 데이터를 관리하였으나 실시간 반영이 되지않았고, 이후 알림, 팔로잉/팔로우, 댓글, 좋아요 수와 같은 실시간 반영이 필요할 경우가 생겨
onSnapshot을 사용해 firestore에 반영되는 정보들을 실시간으로 구독하는 구조로 변경하였습니다.

다만 구독이 중복될 경우 불필요한 리렌더링이 발생하기때문에 useEffect cleanup을 통해 unsubscribe 처리를 하여 중복을 방지하였습니다.
...
Zustand로 관리하는 이유는 Redux 대비 보일러플레이트가 적고, 현재의 프로젝트 규모에 적합한 경량 상태 관리가 가능하다고 생각했기때문입니다.

Firestore를 직접 호출했던 이전의 프로젝트와 달리 service로 분리해 유지보수가 용이하도록 하였습니다.
...
Hook뿐 아니라 반복적으로 사용되는 fetching, filter, modal, slider 로직을 따로 분리하였고, 유지보수뿐 아니라 컴포넌트는 렌더링 역할만, 스타일은 스타일 적용의 역할만 가져갈 수 있도록 내부의 기능적 구분감을 주고자 하였습니다.
